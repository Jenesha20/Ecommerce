...
export const getProducts = (keyword, price, category, rating, currentPage) => async (dispatch) => {
    try {  
        ...
        if(price) {
            link += `&price=${price[0]},${price[1]}`; // Changed format
        }
        if(category) {
...
//         if (price) link += `&price=${price[0]},${price[1]}`;
...
//     }
// };
// // [Keep all your other existing action methods unchanged below...]
// // Action to get product details by ID
// export const getProduct = (id) => async (dispatch) => {
//     try {  
//         dispatch(productRequest()); 
//         const { data } = await axios.get(`/api/v1/product/${id}`);
...
// // Action to get product reviews for admin
// export const getReviews = (id) => async (dispatch) => {
//     try {  
//         dispatch(reviewsRequest()); 
//         const { data } = await axios.get("/api/v1/admin/reviews", { params: { id } });
